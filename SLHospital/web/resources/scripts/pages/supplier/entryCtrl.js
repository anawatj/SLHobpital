 ...
 */
app.controller('supplierEntryCtrl', function (
        $scope,
        $http,
        $pageUtils,
        $dialogUtils,
        ...
        )
{
    ...
    $scope.init = function ()
    {
        ...
        {
            $scope.$parent.pageName="สร้าง-Supplier";
            
        }else
        {
            $scope.$parent.pageName="แก้ไข-Supplier";
        }
        $scope.$parent.link="ค้นหา-Supplier";
        $scope.$parent.urlLink=url+"supplier/list.html";
    ...
    $scope.load = function ()
    {
        ...
        var promise = $q.all(
                [
                    ...
                    $http.get(url + "supplier/getByKey", {params: {key: id}})
                ]
                ).then(function (data)
        {
    ...
    $scope.save = function ()
    {
        $http.post(url + "supplier/save", $scope.model)
                .success(function (data)
                {
                    window.location = url + "supplier/entry.html?id=" + data.id;
                });
    };
}
);
